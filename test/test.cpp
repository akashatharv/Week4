...
class MockPID : public PID{
 public:
  ...
  MOCK_METHOD3(inputGains,void(double inputKp,double inputKi, double inputKd));
  MOCK_METHOD2(computeVelocity, double(double targetSetpoint, double velocity));
};

TEST(pidCompute, ComputationCheck) {
  MockPID obj;   /*!< Object of the class PID  */
  EXPECT_CALL(obj,computeVelocity(40.00,40.00)).WillOnce(Return(0.0)).WillOnce(Return(0.0));
  std::cout<<"The function returns:"<<obj.computeVelocity(40,40)<<std::endl;
